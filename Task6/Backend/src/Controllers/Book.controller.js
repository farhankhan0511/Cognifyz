...
const CancelTour=asynchandler(async(req,res)=>{
    ...
    if(!existbooking){
        return res.status(400).json( new ApiResponse(404,{},"Booking Doesn't exist")
    )}
...
try {
    ...
    return res.status(400).json( new ApiResponse(500,{},error.message || "Something went wrong while cancelling the tour")
)}
res.status(200).json(
